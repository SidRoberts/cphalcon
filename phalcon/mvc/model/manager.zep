...
class Manager implements ManagerInterface, InjectionAwareInterface, EventsAwareInterface
{
	...
	/**
	 * Checks if a record already exists or not
	 */
	public function exists(<ModelInterface> model) -> boolean
	{
		int numberEmpty, numberPrimary;
		var metaData, connection,
			uniqueParams, uniqueTypes, uniqueKey, columnMap, primaryKeys,
			wherePk, field, attributeField, value, bindDataTypes,
			joinWhere, num, type, schema, source, table;
		...
		if uniqueKey === null {
			 ...
			 */
			let model->_uniqueKey = joinWhere,
				model->_uniqueParams = uniqueParams,
				model->_uniqueTypes = uniqueTypes,
				uniqueKey = joinWhere;
		}
		...
		if uniqueTypes === null {
			let uniqueTypes = model->_uniqueTypes;
		}

		let schema = model->getSchema(), source = model->getSource();
		if schema {
			let table = [schema, source];
		} else {
			let table = source;
	 ...
	 * @return boolean
	 */
	protected function _doLowInsert(<ModelInterface> model, <MetaDataInterface> metaData, <AdapterInterface> connection,
	 ...
	 protected function _doLowUpdate(<ModelInterface> model, <MetaDataInterface> metaData, <AdapterInterface> connection, var table) -> boolean
	{
		 ...
		 */
		let useDynamicUpdate = (boolean) manager->isUsingDynamicUpdate(model);

		let snapshot = model->_snapshot;
		...
		let uniqueKey = model->_uniqueKey,
			uniqueParams = model->_uniqueParams,
			uniqueTypes = model->_uniqueTypes;
		...
		if success && model->isKeepingSnapshots(model) {
			if typeof snapshot == "array" {
				let model->_oldSnapshot = snapshot;
				let model->_snapshot = array_merge(snapshot, newSnapshot);
			} else {
				let model->_oldSnapshot = [];
				let model->_snapshot = newSnapshot;
			}
		}
